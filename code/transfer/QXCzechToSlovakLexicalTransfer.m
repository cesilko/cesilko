	...
	if (self = [super init]) {
		if (basicDict == nil) {
			...
			while (fgets(line, 1000, file) != NULL) {
				...
				NSString* slovakLemma = [[[tokens objectAtIndex: 1] trim] lowercaseString];
				...
				[basicDict setObject: [NSDictionary dictionaryWithObjectsAndKeys: slovakLemma, @"lemma", nil] forKey: czechLemma];
			}
			fclose(file);
			[pool release];
