...
int main(int argc, char *argv[]) {
	...
	if (!strcmp(argv[1], "czsk")) {
		lextr = [[[CSCzechToXLexicalTransfer alloc] initWithFile: [NSString stringWithUTF8String: argv[2]]] autorelease];
		strtr = [[[CSCzechToXStructuralTransfer alloc] init] autorelease];
	...
	if (!strcmp(argv[1] + 2, "sk")){
		mg = [[[CSSlovakMorphologicalGenerator alloc] initWithFile: [NSString stringWithUTF8String: argv[3]]] autorelease];
	}
	if (!strcmp(argv[1] + 2, "pl")){
		mg = [[[CSPolishMorphologicalGenerator alloc] initWithFile: [NSString stringWithUTF8String: argv[3]]] autorelease];
	}
	if (!strcmp(argv[1] + 2, "ru")){
		mg = [[[CSRussianMorphologicalGenerator alloc] initWithFile: [NSString stringWithUTF8String: argv[3]]] autorelease];
	}
	if (!strcmp(argv[1] + 2, "de")){
		mg = [[[CSGermanMorphologicalGenerator alloc] initWithFile: [NSString stringWithUTF8String: argv[3]]] autorelease];
	}
	...
	while (bundle = [enumerator nextObject]) {
		BOOL translated = NO;
		unsigned n = [[bundle objectAtIndex: 0] unsignedIntValue];
		NSLog(@"sentence %u", n);
		NSArray* source = [bundle objectAtIndex: 1];
		printf("@%u %s\n", n, [[NSString stringWithArray: source] UTF8String]);
		NSArray* dicts = [bundle objectAtIndex: 2];
		//NSLog(@"%@", dicts);
		NSLog(@"translating...");
